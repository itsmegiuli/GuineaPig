...
class Interactable {
  ...
  constructor(interactables, scene, geometry, position) {
    ...
    // Trigger
    this.collider = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
      color: 0xffffaa,
      transparent: true,
      opacity: 0.15,
      visible: false
  ...
  /**
   * Removes collider from scene and interactable from the list. Make sure there are no other references so gc can take over.
   */
  dispose() {
  ...
  /**
   * Is called when the interactable object is in range but not yet targeted.
   */
  onActivate() {
  ...
  /**
   * Is called when the interactable object is out of range.
   */
  onDeactivate() {
    if (!this.isActive) return;
  ...
  /**
   * Is called when the interactable object is in range and targeted.
   */
  onSelected() {
  ...
  /**
   * Is called when the interactable object in range and no longer targeted.
   */
  onDeselected() {
    if (!this.isSelected) return;
  ...
  /**
   * Is called when the interactable object is triggered
